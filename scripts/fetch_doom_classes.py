#!/usr/bin/env python3
"""
Script to fetch Doom class names from ZDoom wiki and generate an enum class with categories.
...
from the Spawnable page and organizes classes by category. It then generates a complete Python module
containing a DoomClass enum with lookup tables and category mappings.

The generated enum includes:
- All Doom classes found on the ZDoom wiki
- Additional hardcoded classes (ItemFog, TeleportFog, Blood, DeadDoomPlayer)
- Helper methods for class lookup and conversion
- Immutable lookup tables for metadata
- ZDoom categories organized by type (Monster, Weapon, Ammo, etc.)
- Reverse mapping from class names to categories

Features:
- Comprehensive caching system to reduce server load
- Cache expiration and validation
- Fallback to cached data when network is unavailable
- Cache statistics and management

Usage:
    python fetch_doom_classes_WIP.py [-o output_file] [-u url] [-s sleep_delay] [--cache-dir cache_dir] [--force-refresh] [--cache-only]
...
def generate_category_dict(
    ...
    """
    Generate the ZDoom category dictionary.

    Args:
        categories: Dictionary with ZDoom categories as keys and lists of class names as values

    Returns:
        str: formatted string containing CATEGORIES and CLASS_TO_CATEGORY mappings
    """
    print("Generating ZDoom category dictionary")

    content = "CATEGORIES = MappingProxyType(\n    {\n"

    # Add each category
    for category, class_names in sorted(categories.items()):
        content += f'        "{category}": {class_names},\n'

    content += "    }\n)\n" + REVERSE_MAPPING
    print(f"Generated ZDoom category dictionary with {len(categories)} categories")
    return content
...
def generate_enum_class(
    ...
    spawn_ids: Dict[str, int],
    identifiers: Dict[str, str],
    output_file: str = "doom_classes.py",
    category_content: str = "",
) -> None:
    """
    Generate a Python enum class from the list of class names with lookup tables.

    This function creates a complete Python module containing:
    - A DoomClass enum with all class names converted to uppercase identifiers
    - Helper methods for class lookup and conversion
    - Immutable lookup tables for DoomEd numbers, Spawn IDs, and Identifiers
    - Additional hardcoded classes (ItemFog, TeleportFog, Blood, DeadDoomPlayer)
    - Thing categories and reverse mapping (if category_content is provided)

    Args:
        class_names: List of Doom class names to include in the enum
        doomed_numbers: Dictionary mapping class names to DoomEd numbers
        spawn_ids: Dictionary mapping class names to Spawn IDs
        identifiers: Dictionary mapping class names to identifiers
        output_file: Output file path for the generated enum (default: "doom_classes.py")
        category_content: Optional string content to append to the end of the file
    """
    class_file_content = ENUM_HEADER[:]

    for class_name in class_names:
        # Convert class name to valid Python identifier
        enum_name = class_name.upper()
        class_file_content += f'    {enum_name} = "{class_name}"\n'

    class_file_content += ENUM_END

    proxy_mapping_start = " = MappingProxyType(\n    {\n"
    proxy_mapping_end = "    }\n)\n\n"

    # Add lookup tables
    for class_name in class_names:
        if class_name in doomed_numbers:
            class_file_content += f'    "{class_name}": {doomed_numbers[class_name]},\n'

    class_file_content += proxy_mapping_end + "SPAWN_IDS" + proxy_mapping_start

    for class_name in class_names:
        if class_name in spawn_ids:
            class_file_content += f'    "{class_name}": {spawn_ids[class_name]},\n'

    class_file_content += proxy_mapping_end + "IDENTIFIERS" + proxy_mapping_start

    for class_name in class_names:
        if class_name in identifiers:
            class_file_content += f'    "{class_name}": "{identifiers[class_name]}",\n'

    class_file_content += proxy_mapping_end[:-1]

    # Append category content if provided
    if category_content:
        class_file_content += "\n\n" + category_content
    ...
    print(f"Generated enum class with {len(class_names)} doom classes in {output_file}")
    print(
        f"Added {len(doomed_numbers)} DoomEd numbers, {len(spawn_ids)} Spawn IDs, {len(identifiers)} Identifiers"
    )


def main() -> None:
    """Main function to fetch classes and generate enum with ZDoom categories."""
    parser = argparse.ArgumentParser(
        description="Fetch Doom class names from ZDoom wiki and generate enum with ZDoom categories (with caching)"
    )
    parser.add_argument(
        "-o",
        "--output",
        default="src/lib_python/doom_classes.py",
        help="Output file path for the generated enum (default: src/lib_python/doom_classes.py)",
    )
    parser.add_argument(
        "-s",
        "--sleep",
        type=float,
        default=30.0,
        help="Delay (in seconds) between each request (default: 30)",
    )
    parser.add_argument(
        "--cache-dir",
        default=".cache",
    ...
    try:
        ...
        # Fetch lookup tables from individual class pages
        ...
        # Generate ZDoom category dictionary
        category_content = ""
        if categories:
            category_content = generate_category_dict(categories)

        # Generate the enum class with lookup tables and category content
        generate_enum_class(
            class_names,
            doomed_numbers,
            spawn_ids,
        ...
        print(f"Successfully generated Doom class enum in {args.output}!")
