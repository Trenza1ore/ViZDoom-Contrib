#!/usr/bin/env python3
"""
Script to fetch Doom class names from ZDoom wiki and generate C++ files with categories.
...
from the Spawnable page and organizes classes by category. It then generates C++ header
and source files containing the required data structures.

The generated C++ files include:
- Header file with extern declarations for the three required variables
- Source file with the actual data definitions
...
Usage:
    python fetch_doom_classes.py [-o output_base] [-cp copy_to] [-s sleep_delay] [--cache-dir cache_dir] [--force-refresh] [--cache-only] [--clear-cache]
...
CPP_HEADER_TEMPLATE = f"""#pragma once
...
// Generated by {os.path.split(os.path.dirname(__file__))[1]}/{os.path.basename(__file__)}
...
def generate_cpp_files(
    ...
    """
    Generate C++ header and source files from the list of class names and categories.

    This function creates:
    - A header file (.h) with extern declarations for the three required variables
    - A source file (.cpp) with the actual data definitions
    ...
    source_file = f"{output_base}.cpp"
    source_content = f"""#include "{os.path.basename(output_base)}.h"
...
"""
    ...
    for class_name in sorted(casefolded_class_names):
        source_content += f'    "{class_name}",\n'

    source_content += "};\n\n"

    # Add category to classes mapping
    source_content += "// Mapping from category names to sets of class names\n"
    source_content += "std::unordered_map<std::string, std::unordered_set<std::string>> categoryToClasses = {\n"

    for category, class_list in sorted(casefolded_categories.items()):
        source_content += f'    {{"{category}", {{\n'
    ...
    source_content += "// Mapping from class names to their category\n"
    source_content += (
        "std::unordered_map<std::string, std::string> classToCategory = {\n"
    )

    for category, class_list in sorted(casefolded_categories.items()):
        for class_name in class_list:
            source_content += f'    {{"{class_name}", "{category}"}},\n'
    ...
    print(
        f"Generated C++ files with {len(casefolded_class_names)} doom classes from {len(casefolded_categories)} categories"
    )
...
def main() -> None:
    """Main function to fetch classes and generate C++ files with ZDoom categories."""
    parser = argparse.ArgumentParser(
        description="Fetch Doom class names from ZDoom wiki and generate C++ files with ZDoom categories (with caching)"
    )
    parser.add_argument(
        "-o",
        "--output",
        default="src/vizdoom/src/viz_doom_classes",
        help="Base name for output files (without extension) (default: src/vizdoom/src/viz_doom_classes)",
    )
    parser.add_argument(
        "-cp",
        "--copy-to",
        default="src/lib",
        help="Copy the generated files to the specified directory (default: src/lib)",
    )
    parser.add_argument(
        "-s",
        "--sleep",
    ...
    try:
        ...
        # Generate C++ files
        generate_cpp_files(class_names, categories, args.output)

        # Print cache statistics
        cache_manager.print_stats()

        print(
            f"Successfully generated C++ files!\n- {args.output}.h\n- {args.output}.cpp"
        )

        if args.copy_to:
            print(f"Copying generated files to {args.copy_to}...")
            subprocess.run(
                ["cp", f"{args.output}.h", f"{args.output}.cpp", args.copy_to],
                check=True,
            )
            print(f"Copied files to {args.copy_to}")
